    ...
    table = $('#tblistadoReg').dataTable(
        {
            ...
            "ajax":{
                url: '../ajax/categoria.php?op=listar',//Establecemos la opción en donde ajax va a interactuar los datos
...
//limpiar formulario
const limpiar = ()=>{
    $("#idCategoria").val(""); //Selecciona con jquery el idDepartamento y pone el valor un string vacio
...
const mostrar = (idCategoria)=>{
    ...
    $.post("../ajax/categoria.php?op=mostrar",{idCategoria:idCategoria},(data)=>{
        //console.log(data);//String en formato json
        data = JSON.parse(data);//transforma el json en objeto
        //console.log(data);//objeto js
        mostrarForm(true);
        $('#idCategoria').val(data.idCategoria);//Selecciona con jquery el idDepartamento y pone el valor idDepartamento del data
...
const desactivar = (idCategoria)=>{
    let ventanaEleccion = toastr.warning("¿Deseas desactivar la categoria seleccionado?"+
    ...
    $("#rptaSi").click(()=>{
        ...
        $.post("../ajax/categoria.php?op=desactivar",{idCategoria:idCategoria},(respuesta)=>{
            let valida = respuesta.indexOf("rror,");//busca la cadena en otra cadena
            if(valida != -1){
                toastr["error"](respuesta);//manda un modal de error con la respuesta
            }else{
...
const activar = (idCategoria)=>{
    let ventanaEleccion = toastr.warning("¿Deseas activar la categoria seleccionado?"+
        "<br><button type='button' id='rptaSi' class='btn btn-success'>SI</button>"+
        "<button type='button' id='rptaNo' class='btn btn-danger'>NO</button>"
        ,"Alerta");
    $("#rptaSi").click(()=>{
        toastr.clear(ventanaEleccion);
        $.post("../ajax/categoria.php?op=activar",{idCategoria:idCategoria},(respuesta)=>{
